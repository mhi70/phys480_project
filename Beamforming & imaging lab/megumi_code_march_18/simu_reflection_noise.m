...
xo = ry*1e-3; % target lateral
z = rx*1e-3; % target depth
xd = array.element.lateral; % detector lateral 
xe = array.element.lateral(25); % emitter lateral
yd = array.element.depth;
ye = array.element.depth(25);
xo = xo + xe(1); % shift 
v = medium.sound_speed_ref;
d1 = sqrt((z-ye)^2+(xo-xe)^2); % distance, object to emitter
d2 = sqrt((z-yd)'.^2+(xo-xd).^2); % distance, object to detector
