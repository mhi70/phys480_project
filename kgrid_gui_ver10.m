...
addLabelField(gridPanel, 'Nx:', 340, 80, 256, 'NxField');
addLabelField(gridPanel, 'y span (mm):', 20, 50, 15, 'ySpanField');
addLabelField(gridPanel, 'Ny:', 340, 40, 256, 'NyField');
...
addLabelField(abPanel, 'Depth (mm):', 250, 60, 5, 'AberrDepthField');
addLabelField(abPanel, 'Thickness (mm):', 250, 35, 2, 'AberrThickField');
addLabelField(abPanel, 'Coefficient:', 250, 10, 1.5, 'AberrCoefField');
toggleAberrFields(fig, 'off')

% Target Panel
targetPanel = uipanel(fig, 'Title', 'Target Properties', 'Position', [20, 300, 660, 120]);
addLabelField(targetPanel, 'Depth rx (mm):', 20, 70, 11, 'rxField');
addLabelField(targetPanel, 'Lateral ry (mm):', 340, 70, 12, 'ryField');
addLabelField(targetPanel, 'Radius r (mm):', 20, 30, 0.45 * 4.7385e-04 * 1e3, 'rField');
addLabelField(targetPanel, 'Reflectivity:', 340, 30, 3, 'reflField');
...
tags = {'AberrDepthField', 'AberrThickField', 'AberrCoefField'};
for i = 1:length(tags)
...
if getVal(fig, 'aberrCheck')
    ...
    % mid_point = round(abs(thick_idx-depth_idx)/2);

    medium.sound_speed(depth_idx:(depth_idx+thick_idx-1), :) = ...
        medium.sound_speed(depth_idx:(depth_idx+thick_idx-1), :) * ab_coeff;
    % 
    % max_speed = ab_coeff * medium.sound_speed_ref;
    % gradient = (max_speed-medium.sound_speed_ref)/abs(thick_idx - depth_idx);
    % linear_function1 = @(x) gradient.*(x-depth_idx) + medium.sound_speed_ref;
    % linear_function2 = @(x) -gradient.*(x-depth_idx) + medium.sound_speed_ref;
    % length1 = depth_idx - (depth_idx+mid_point);
    % length2 = depth_idx- depth_idx+mid_point;
    % diff = length1 - length2;
    % medium.sound_speed(depth_idx:depth_idx+mid_point,:)=(medium.sound_speed(depth_idx:depth_idx+mid_point,:)'.*linear_function1(depth_idx:depth_idx+mid_point))';
    
    % length1 = depth_idx+mid_point-(depth_idx+thick_idx-1);
    % length2 = depth_idx-(depth_idx+mid_point);
    % diff = length1 - length2;
    % medium.sound_speed(depth_idx+mid_point:depth_idx+thick_idx-1,:)=(medium.sound_speed(depth_idx+mid_point:depth_idx+thick_idx-1,:)'.*linear_function2(depth_idx:depth_idx+mid_point-diff))';
...
uibutton(fig, 'Text', 'Run US Simulation', 'Position', [250, 120, 200, 30], ...
    'ButtonPushedFcn', @(btn, event) runUS_Simulation(fig, fig2, medium_US));
uibutton(fig, 'Text', 'Run PA Simulation', 'Position', [470, 120, 200, 30], ...
    'ButtonPushedFcn', @(btn, event) runPA_Simulation(fig,medium_PA));
% Output Panel
outputPanel = uipanel(fig, 'Title', 'Output Settings', 'Position', [20, 200, 660, 80]);
addLabelTextField(outputPanel, 'US Output File:', 20, 30, 'K_US.mat', 'usFileField');
addLabelTextField(outputPanel, 'PA Output File:', 340, 30, 'K_PA.mat', 'paFileField');
end
...
usFileField = findobj(fig, 'Tag', 'usFileField');
usFileName = usFileField.Value;
if isfile('output/'+""+usFileName)
    selection = uiconfirm(fig, ['The file "', usFileName, '" already exists. Overwrite?'], 'File Exists Warning', ...
...
filename = 'output/'+""+getVal(fig, 'usFileField');
save(filename,'M','array','pulse','pml','medium','kgrid'); % save the dataset!

end


function runPA_Simulation(fig,medium_PA)
global kgrid pml medium pulse target source

% check if file exists
paFileField = findobj(fig, 'Tag', 'paFileField');
paFileName = paFileField.Value;
if isfile('output/'+""+paFileName)
    selection = uiconfirm(fig, ['The file "', paFileName, '" already exists. Overwrite?'], 'File Exists Warning', ...
        'Options', {'Overwrite', 'Cancel'}, 'DefaultOption', 2, 'CancelOption', 2);
    if strcmp(selection, 'Cancel')
        return;
    end
...
filename = 'output/'+""+getVal(fig, 'paFileField');
save(filename,'K_PA','array','medium_PA','source','pulse','pml','medium','kgrid')
end