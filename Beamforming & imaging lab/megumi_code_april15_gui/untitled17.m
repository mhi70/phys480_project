...
% Hide the main GUI window (but don't delete it)
set(fig, 'Visible', 'off');

% Create a new figure for plotting the sound speed
figure(fig2);  % Make sure to direct plots to fig2
hold on;
imagesc(kgrid.y_vec * 1000, kgrid.x_vec * 1000, medium.sound_speed);
title('Sound Speed (m/s)');
xlabel('y: Lateral distance (mm)', 'Interpreter', 'latex', 'FontSize', 10);
...
% Create a figure for the pulse signal
fig3 = figure('Name', 'Input Pulse - Time Domain', 'NumberTitle', 'off');
...
array.element.emission = 1;  % Make sure the transducer emits
[K, array, ~] = run_simulation(array, [], true);

% Optionally, you can further process the simulation results here
