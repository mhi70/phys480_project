...
%       optional paramter plotflag : default is 0. Set to ==1 to plot the image and an Rxx slice for each emission and reception (slower but fun to watch)
%    for PW emission:
...
%
% - depth defined wrt array - no info about transducer depth will be included in this function
% - kgrid and array no longer required for input - all info is in Img structure (or R)
...
element_num = size(R,2);  % # elements in array

if(PWflag)
    emission_num=length(angles);
else
    emission_num=element.num; % just over emission elements
end

trans_pos_x = Img.trans_pos_x; % [m]
