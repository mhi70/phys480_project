...
% xe = array.element.lateral(array.element.emission)*1000; % emitter lateral [mm]
% xo = ry;
% xo = xo + kgrid.y_vec(1)*1000; % shift  [mm]
% v = medium.sound_speed_ref*1000; %[mm/s]
% angle_x = cos(20*pi/180);
% angle_y = sin(20*pi/180);
% d= sqrt(abs((xo - xe)).^2+rx^2); %[mm]
% d_angle = sqrt(abs((xo - xe)*angle_x).^2+(rx*angle_y)^2); %[mm]
% delta_d = d - d_angle;
% delays = delta_d/v; % [s]
...
for xx = 1:array.nb_sources % for each detection element lateral position
    % location of element
    element_x = array.element.lateral(xx); % [m], element lateral position
    element_z = array.element.depth(xx); %[m], element depth


    for zz = 1: Nx % loop over image pixels in depth
        
        R = squeeze(K(xx,:));

        target_x = element_x ; % same as element lateral position
        target_z = kgrid.ky_vec(zz)/1000 + kgrid.ky_vec(1)/1000; % [m]

        transmit_distance = sqrt((emitter_x - target_x)^2+(emitter_z - target_z)^2)/1000; % [m], emitter to target
        receive_distance = sqrt((element_x - target_x)^2+(element_z - target_z)^2)/1000 ; % [m], target to element

        time_delay = (transmit_distance + receive_distance)/medium.sound_speed_ref; % [s] time delay at point
        ...
        img_data(xx, zz)=img_data(xx, zz)+tmp;
    end

    % you can watch how the image forms with each subsequent detection: 
    imagesc(kgrid.kx_vec/1000, kgrid.ky_vec/1000, abs(img_data))
    ylabel('depth (mm)')
...
for xx = 1:array.nb_sources % for each detection element lateral position
    ...
    % for each element 
    for zz = 1: Nx % loop over image pixels in depth
        R = squeeze(K(xx,:)); % the depth pixels at lateral position xx
        target_x = element_x ; % [m] same as element lateral position
        target_z = kgrid.ky_vec(zz)/1000 + kgrid.ky_vec(1)/1000; % [m]
